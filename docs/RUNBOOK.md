...
| Gateway status says `fetch failed` and the key is one byte | Ctrl-V was captured as a control character by PowerShell's hidden prompt | Rerun `setup.cmd`, paste the replacement key with right-click or Shift+Insert, and do not press Ctrl-V. Current installers reject malformed/non-`sk-` keys. |
