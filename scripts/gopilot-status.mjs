...
import { analyzePiSessions } from "../src/observability/pi-cache.mjs";
...
// Pi's status-bar CH is the latest call, while the arrows are cumulative. Keep
// both views explicit and report the same-model, short-gap calls eligible for
// the 98% warm-cache aspiration.
status.piCache = analyzePiSessions([
  resolve(homedir(), ".pi", "agent", "sessions"),
  resolve(homedir(), ".local", "share", "gopilot", "pi-sessions"),
]);
...
if (existsSync(ledgerPath)) {
  ...
  for (const model of [...new Set(entries.map((e) => e.model))]) {
    ...
    status.delegations.models[model] = {
      ...
      usageKnownRate: ms.length ? +(ms.filter((e) => e.usage && Number.isFinite(e.usage.total ?? e.usage.totalTokens)).length / ms.length).toFixed(3) : null,
      routingOverrides: ms.filter((e) => e.suggested && e.suggested !== e.model).length,
    };
    const latestWithUsage = [...ms].reverse().find((e) => e.usage);
    const latestFresh = latestWithUsage?.usage?.in ?? latestWithUsage?.usage?.input ?? 0;
    const latestCached = latestWithUsage?.usage?.cached ?? latestWithUsage?.usage?.cacheRead ?? 0;
    status.delegations.models[model].latestCacheHitPct = latestFresh + latestCached > 0
      ? +(100 * latestCached / (latestFresh + latestCached)).toFixed(1) : null;
  }
}
...
if (asJson) {
  ...
  for (const [m, s] of Object.entries(d.models)) {
    ...
    console.log(`    ${m.padEnd(9)} ok ${(s.okRate * 100).toFixed(1)}% · p50 ${s.latencyMs.p50}ms p95 ${s.latencyMs.p95}ms · repairs ${s.repairAttempts} · tok in/out/reason/cached ${s.tokens.in}/${s.tokens.out}/${s.tokens.reasoning}/${s.tokens.cached} (cache cumulative ${hitPct}% · latest ${s.latestCacheHitPct ?? "n/a"}% · usage known ${(100 * s.usageKnownRate).toFixed(1)}%)${s.routingOverrides ? ` · overrides ${s.routingOverrides}` : ""}`);
  }
  const pc = status.piCache;
  console.log(`  pi cache   cumulative ${pc.cumulative.hitPct ?? "n/a"}% · latest ${pc.latest?.hitPct ?? "n/a"}% · eligible warm ${pc.eligibleWarm.hitPct ?? "n/a"}% (${pc.eligibleWarm.at98Pct ?? "n/a"}% of calls >=98%)`);
