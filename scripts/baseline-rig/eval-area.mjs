...
function armA(dir) {
  const graded = readJsonl(join(HERE, dir, "graded-runs.jsonl")).filter((r) => r.arm === "A" && FIXTURES.includes(r.fixtureId));
  const runs = Object.fromEntries(readJsonl(join(HERE, dir, "campaign-runs.jsonl")).filter((r) => r.arm === "A" && FIXTURES.includes(r.fixtureId)).map((r) => [r.key, r]));
  const byFix = {};
...
let medians = [], deepMedians = [], totFails = 0, totRuns = 0, totCost = 0, totGraded = 0, maxLat = 0, hardReg = 0, minorReg = 0;
...
const gates = [
  ...
  [`Reliability 100% (0 fail / ${totRuns} runs)`, `${totFails === 0 ? "100" : r1(100 * (totRuns - totFails) / totRuns)}%`, totFails === 0],
