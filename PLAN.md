...
### Sprint 8 — Production Readiness / Integrated Control Plane  ·  progress `Phase A 7/9 + Phase B core done`  ·  *(Phase A 8.1–8.7 ✅; Phase B 8.10 IPC, 8.11 reliability-core, 8.12 observability, 8.13 config-governance ✅ — 293/293. Remaining = user/infra-gated only: 8.8 live-workhorse [Ikey], 8.9 benchmark [fixtures+quota], 8.14 prod-security [Mem0 services], 8.15 clean-machine [boxes], 8.16 pilot; + 2 herdr-gated follow-ons [wire reliability into run.mjs; live alert sink])*
...
- [x] **Step 8.10: Hardened IPC (agent-comms)** [Complex] ✅ 2026-07-11 — Depends on Step 3.7 · per-run auth token (timing-safe compare), message-size cap, connection cap + idle-eviction, in-flight concurrency cap, correlation IDs, bounded sanitized errors. Security Engineer review found a connection-slot lockout (no idle timeout → cheap DoS) + unbounded client reply buffer; both fixed + tested. Done when: unauthorized peers + oversized messages are rejected in automated tests. → **mesh 14/14 (5→14), no flakes.**
- [~] **Step 8.11: Reliability & recovery** [Complex] ✅ (hermetic core) 2026-07-11 — Depends on Step 8.1, 8.6 · `src/reliability/retry.mjs` (backoff+jitter, abortable withRetry, circuit breaker — all injectable clock/rng/sleep) + `src/reliability/journal.mjs` (durable run/task IDs, crash-tolerant append-only JSONL, **idempotent dispatchOnce** [no double-execute], **reconcile** [no lost in-flight work]). retry 17/17 + journal 17/17, no flakes. Done when: killing the orchestrator at each stage recovers with no lost/duplicated work → core proven in hermetic tests. FOLLOW-ON (herdr-gated, like 8.8): wire journal+retry+breaker into `run.mjs` dispatch + pane/worktree cleanup for a live end-to-end kill test.
- [x] **Step 8.12: Production observability** [Complex] ✅ 2026-07-11 — Depends on Step 8.1 · `src/observability/events.mjs`: structured events (injected clock + seq), `inspect(runId)` (timeline/totals/failures/retries/slowest/most-expensive), `aggregate(dimension)` (by model/category/profile/provider → count/tokens/cost/okRate/retryRate/latency median+p90, reuses stats.mjs), key-based `redact()` (secrets never reach all()/sink). events 19/19. Done when: a failed/expensive run is diagnosable from its run ID → met. (Live alerting/sink wiring is a herdr-gated follow-on.)
- [~] **Step 8.13: Config & model governance** [Medium] ✅ (core) 2026-07-11 — Depends on Step 3.1 · model registry `config/models.json` (pinned versions + capability metadata + active flags); `src/config/governance.mjs` validates router+registry (schema, plane validity, alias existence, **fail-closed on inactive model**, plane-mismatch); `gopilot config doctor` CLI (exit 1 on any error); `resolveModel()` helper for per-run provider/version. Done when: invalid/incomplete profiles fail before dispatch → **governance 13/13, `config doctor` green on shipped config.** FOLLOW-ON (deferred, additive): wire `resolveModel` into `run.mjs` to record resolved provider/version on each run's plan.
